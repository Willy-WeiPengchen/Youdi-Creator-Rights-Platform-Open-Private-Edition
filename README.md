# Youdi-Creator-Rights-Platform-Open-Private-Edition  
The First Implementable Protocol Framework for Digital Creator Sovereignty · Private Alliance Edition · A Historic Social-Technical Experiment

---

“基于本核心协议构建的完整平台参考模型，请参阅：Youdi-IP-Curation-Socialization-Model”
https://github.com/Willy-WeiPengchen/Youdi-IP-Curation-Socialization-Model

---

🏛️ 优谛创作者权益平台 · 开源私享版
Youdi-Creator-Rights-Platform-Open-Private-Edition（Youdi-CRB-PE）

数字时代的创作者主权宣言 · 全球首个可实施的AI数据伦理协议框架

历史起点：2025年12月31日 · 于中国今日头条  《以创作者（权益）银行为纽带，畅通数字经济“产贸销用”循环》https://www.toutiao.com/article/7589952933102404146/

🌊 项目定位
本项目是 “优谛创作者权益平台”体系的开源私享版（Private Edition）技术实现框架，旨在为全球开发者、平台方与创作者提供一套可立即部署、可自主演进的数字创作者权益保护基础协议。

🔄 双轨治理模式
- **开源层（Open Layer）**：基于 Apache License 2.0 开放基础确权、溯源协议与部署工具，鼓励全球协作。
- **闭源母版（Core Sovereign Layer）**：智能分账算法、跨境合规引擎等核心模块以专利保护形式保留，通过标准化 API 提供生态对接。

📜 核心原则
创作者至上：原创者对其作品拥有完整的署名权、解释权与收益分配权

开放共治：通过“国家指导 + 联席主席委员会 + 创作者权益理事会”三方共治架构实现生态治理

透明可分账：基于智能合约的可验证收益分配机制，杜绝黑箱操作

合规可适配：严格遵循各国数据安全与版权法规，提供跨境合规解决方案

🧩 项目结构
text
Youdi-Creator-Rights-Platform-Open-Private-Edition/
├── README.md                      # 项目首页
├── white-paper-zh.md              # 完整中文白皮书
├── patent-trademark-notice.md     # 专利与商标保护声明
└── LICENSE                        # Apache 2.0 官方许可证

🚀 快速开始
克隆仓库

bash
git clone https://github.com/your-org/Youdi-Creator-Rights-Platform-Open-Private-Edition.git

阅读白皮书

中文白皮书

英文白皮书

部署私享版

（可选）申请母版生态对接

满足《母版开放路线图》条件后，可申请接入优谛全球协作网络

🌍 全球协作
议题讨论：通过 GitHub Issues 提交功能建议与生态共建提案

版本迭代：每季度发布正式版本，遵循语义化版本规范

学术合作：欢迎研究机构基于本框架开展数字权益、AI伦理相关研究

⚖️ 免责声明

本项目为 技术框架与治理理念的开源实现，不构成任何法律或投资建议

部署方须自行确保其使用符合所在地法律法规

母版（闭源核心）相关专利授权需另行签署《共建对价承诺书》并获得书面授权

⚠️ 特别提示：

- 本框架不自动授予任何专利实施许可；
  
- “优谛”为注册商标，“优谛”商标仅限于描述本项目，未经许可不得用于暗示官方背书或商业推广。

📬 联系与致敬

原创架构师：韦朋辰（Wei Pengchen）  
历史时刻：2025年12月31日于今日头条首次提出“创作者权益保护体系”概念原型（初期曾以“创作者权益银行”作为比喻性表述，后正式定名为“优谛创作者权益平台”）  
开源日期：2026年2月1日  

我们不是在编写代码，而是在为数字文明书写第一部《创作者权利宪章》。  
This is not just code. It is the first draft of a Creator’s Bill of Rights for digital civilization.

📌 免责声明

本项目为社会技术实验协议，所有实施行为均由接入方、共建方自行承担风险；项目方仅提供规则、协议与技术指引，不承担因违规使用、实施不当带来的任何法律、经济责任。

本人作为本项目倡议者，仅负责发布本方案及相关文档，不提供任何形式的技术、咨询、对接等服务，不参与项目任何共建、实施环节，不承担项目开源后产生的任何法律、经济、技术等相关责任，无任何相关负担。

“全球首个”特指：  

**首个同时满足以下三要素的开源治理框架**：  
① **可实施** —— 基于 Apache 2.0 许可证公开可部署；  
② **创作者权益三位一体** —— 覆盖署名权、智能分账权、跨境合规权；  
③ **内置治理引擎** —— 支持“国家指导 + 产业协同 + 创作者共治”的三方架构。

（参见：中华网报道《中国独立研究者发布全球首个可实施的创作者权益治理框架》，2026-02-03）
https://mtz.china.com/touzi/2026/0203/216412.html

---

✨ 数字文明的火种，源于每一位创作者的本真表达

韦朋辰 (Pengchen Wei) 

优谛创作者权益平台 · 私享版开源协议框架 架构师
中国 · 河北
